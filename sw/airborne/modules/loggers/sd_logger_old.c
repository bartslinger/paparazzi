  ...
  switch(sd_logger.state) {
    ...
    case SdLoggerStateRequestingData:
      if (sd_logger.spi_t.status == SPITransSuccess || sd_logger.spi_t.status == SPITransDone) {
        sdcard_send_cmd(17, sd_logger.read_address, SdResponseNone, &sd_logger_request_single_byte);
    ...
    case SdLoggerStateRecording:
      ...
      if (sd_logger.imu_buffer_idx >= 6+(512/SD_LOGGER_SINGLE_RECORD_SIZE)*SD_LOGGER_SINGLE_RECORD_SIZE && sd_logger.sd_busy == 0) {
        ...
        sd_logger.try_counter = 0;
        sd_logger.imu_buffer_idx = 6;
        sdcard_send_cmd(24, 0x00000000, SdResponseNone, &sd_logger_request_single_byte);
      }
...
void sdcard_send_cmd(uint8_t cmd, uint32_t arg, enum SdResponseType response_type, SPICallback after_cb)
{
  sd_logger.spi_t.select = SPISelectUnselect;
...
void sd_logger_send_CMD0(struct spi_transaction *t)
{
  ...
  sdcard_send_cmd(0, 0x00000000, SdResponseR1, &sd_logger_get_CMD0_response);
}
...
void sd_logger_get_CMD0_response(struct spi_transaction *t)
{
  ...
  if(sd_logger_get_R1() == 0x01){
    ...
    sdcard_send_cmd(8, 0x000001AA, SdResponseR7, &sd_logger_process_CMD8);
  }
  else {
...
void sd_logger_process_ACMD41_SDv2(struct spi_transaction *t)
{
  ...
  switch(response) {
    case 0x00:
      ...
      sdcard_send_cmd(58, 0x00000000, SdResponseR3, &sd_logger_process_CMD58);
      break;
    case 0x01:
      break;
    default:
...
void sd_logger_process_ACMD41_SDv1(struct spi_transaction *t)
{
  ...
  switch(response) {
    case 0x00:
      ...
      sdcard_send_cmd(16, 0x00000200, SdResponseR1, &sd_logger_process_CMD16);
      break;
    case 0x01:
      break;
    default:
