...
/** @file filters/heli_rate_filter.c
 *  @brief First order low-pass filter with delay
 *
 * This is a first order low-pass filter with configurable bandwidth frequency.
 * For the purpose of state estimation, a delay is built in. This is the number
 * of time steps that the signal is delayed. An internal buffer is used for this
 * which has a maximum size defined in HELI_RATE_FILTER_BUFFER_SIZE.
 *
 */
 ...
 */
void heli_rate_filter_initialize(struct heli_rate_filter_t *f, uint32_t omega, uint8_t delay, uint16_t max_inc)
{
  /* Set delay */
 ...
 * buffer.
 */
int32_t heli_rate_filter_propagate(struct heli_rate_filter_t *f, int32_t input)
{
 ...
 */
void heli_rate_filter_set_omega(struct heli_rate_filter_t *f, uint32_t omega)
{
 ...
 */
void heli_rate_filter_set_delay(struct heli_rate_filter_t *f, uint8_t delay)
{
