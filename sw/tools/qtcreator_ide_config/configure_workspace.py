...
PAPARAZZI_HOME = os.path.realpath(os.path.join(TOOL_DIR, os.pardir, os.pardir, os.pardir))
...
if not os.path.isfile(os.path.join(TEST_HOME, AIRCRAFT + "Tester.creator")):
	print "No test environment configured for this aircraft. To do so, create a project called \"" + AIRCRAFT + "Tester\" in " + TEST_HOME + "."
	TEST_CONF = False
else:
	TEST_CONF = True
...
# If the gcc_interpeter.py runs for the first time, it regenerates these files. Only the first time
...
cmd = "make -f Makefile.ac PAPARAZZI_HOME=" + PAPARAZZI_HOME + " PAPARAZZI_SRC=" + PAPARAZZI_HOME + " AIRCRAFT=" + AIRCRAFT + " CC='python " + os.path.join(TOOL_DIR, "gcc_interpeter.py") + " " + AIRCRAFT + "' CP='python " + os.path.join(TOOL_DIR, "skip_command.py") + "' LD='python " + os.path.join(TOOL_DIR, "skip_command.py") + "' DMP='python " + os.path.join(TOOL_DIR, "skip_command.py") + "' NM='python " + os.path.join(TOOL_DIR, "skip_command.py") + "' NPROCS=1 ap.compile"
...
if TEST_CONF:
	...
	test_config_file = open(os.path.join(TEST_HOME, AIRCRAFT + "Tester.config"))
	macro_text = test_config_file.read()
	test_config_file.close()
	macro_flags = macro_text.split("\n")
	for i, s in enumerate(macro_flags):
		# defines in <AIRCRAFT>.yml
		macro_flags[i] = "    - -D" + s.replace("#define ", "").replace(" ", "='") + "'"

	macro_flags.pop()
	yml_macro_flags = "\n".join(macro_flags)

	# Get include directories from testing includes file
	test_includes_file = open(os.path.join(TEST_HOME, AIRCRAFT + "Tester.includes"))
