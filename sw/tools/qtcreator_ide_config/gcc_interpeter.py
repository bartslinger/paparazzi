...
# Only look at gcc command in which compilation of an object takes place. This has the -MMD flag.
...
if not os.path.isfile(os.path.join(PAPARAZZI_HOME, AIRCRAFT + ".config")):
	...
	if TEST_CONF:
		test_config   = open(os.path.join(TEST_HOME, AIRCRAFT + "Tester.config"), "w")
		test_includes = open(os.path.join(TEST_HOME, AIRCRAFT + "Tester.includes"), "w")
	...
	for argument in sys.argv:
		...
		if(matchObj) :
			...
			if TEST_CONF:
				...
				newInclude = os.path.normpath(os.path.join(TEST_HOME, "sw", "airborne", matchObj.group(1)))
				test_includes.write(newInclude + "\n")
	# end for loop
	...
	if TEST_CONF:
		...
		test_includes.write(os.path.join(TEST_HOME, "unity", "extras", "fixture", "src") + "\n")
...
for argument in sys.argv:
	if(argument.endswith('.c')):
		...
		cmd = "gcc -MM " + c_file + " " + includeflags + " -include" + os.path.join(PAPARAZZI_HOME, AIRCRAFT + ".config")
