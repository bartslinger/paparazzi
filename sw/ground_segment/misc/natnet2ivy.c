...
gboolean timeout_transmit_callback(gpointer data) {
  ...
  for(i = 0; i < MAX_RIGIDBODIES; i++) {
    ...
    if (startsec == 0) startsec = now.tv_sec;
    //printf("%d.%d\n", (unsigned int)now.tv_sec, (unsigned int)now.tv_nsec);
    printf("%ld.%09d\t%f\t%f\t%f\t%f\t%f\t%f\n", (long) now.tv_sec-startsec, (int)now.tv_nsec,
                                    DegOfRad(roll),
                                    DegOfRad(pitch),
                                    DegOfRad(myheading),
                                    (double)rigidBodies[i].x*100.0,
                                    (double)rigidBodies[i].y*100.0,
                                    (double)rigidBodies[i].z*100.0);
